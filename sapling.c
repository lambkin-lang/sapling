...
static struct ScratchMark txn_scratch_mark(struct Txn *txn)
{
    ...
    if (mark.seg) mark.used=mark.seg->used;
    return mark;
}
...
static void *txn_scratch_alloc(struct Txn *txn, uint32_t len)
{
...

    {
        ...
        if (!seg->buf){ free(seg); return NULL; }
        seg->cap=cap;
        seg->used=n;
...
static int key_has_prefix(const void *key, uint32_t key_len,
                          const void *prefix, uint32_t prefix_len)
{
    if (prefix_len>key_len) return 0;
...
static int txn_track_change(struct Txn *txn, uint32_t dbi,
                            const void *key, uint32_t key_len)
{
    ...
    if (!txn) return -1;
    if (!txn->track_changes) return 0;
    if (dbi>=txn->db->num_dbs) return -1;
    if (txn->db->dbs[dbi].flags&DBI_DUPSORT) return 0;
    if (!key&&key_len>0) return -1;

    for (uint32_t i=0;i<txn->change_cnt;i++) {
        const struct TxnChange *cur=&txn->changes[i];
        if (cur->dbi!=dbi||cur->key_len!=key_len) continue;
    ...
    if (txn->change_cnt>=txn->change_cap) {
        ...
        struct TxnChange *na=(struct TxnChange*)realloc(txn->changes,
                                                        (size_t)nc*sizeof(*na));
        if (!na) return -1;
        txn->changes=na;
        txn->change_cap=nc;
...
static struct WatchRec *watch_snapshot_locked(const struct DB *db, uint32_t *count_out)
{
    ...
    if (!db||!db->num_watches) return NULL;
...
static void txn_notify_watchers(struct Txn *txn,
                                const struct WatchRec *watch_snap,
                                uint32_t watch_count)
{
    ...
    for (uint32_t i=0;i<txn->change_cnt;i++) {
        ...
        for (uint32_t w=0;w<watch_count;w++) {
            if (!watch_snap[w].cb) continue;
            if (watch_snap[w].dbi!=chg->dbi) continue;
            if (!key_has_prefix(chg->key,chg->key_len,
                                watch_snap[w].prefix,watch_snap[w].prefix_len))
                continue;
...
static int user_keycmp(const struct DB *db, uint32_t dbi,
                       const void *a, uint32_t al, const void *b, uint32_t bl)
{
    if (db->dbs[dbi].cmp)
...
static int user_valcmp(const struct DB *db, uint32_t dbi,
                       const void *a, uint32_t al, const void *b, uint32_t bl)
{
    if (db->dbs[dbi].vcmp)
...
static int keycmp(const struct DB *db, uint32_t dbi,
                  const void *a, uint32_t al, const void *b, uint32_t bl)
{
...
static uint32_t raw_alloc(struct Txn *txn)
{
    ...
    if (pgno>=db->pages_cap) {
        ...
        if (db->num_old_arrays>=db->cap_old_arrays) {
            ...
            if (oa){ db->old_page_arrays=oa; db->cap_old_arrays=oc; }
        }
        if (db->num_old_arrays<db->cap_old_arrays)
...
static int meta_load(struct DB *db)
{
    ...
    if (max_dbs>SAP_MAX_DBI) max_dbs=SAP_MAX_DBI;
    int ok0=0, ok1=0;
    ...
    if (db->num_dbs==0 || db->num_dbs>max_dbs) return -1;
    uint32_t off=META_DBS;
    for (uint32_t i=0;i<db->num_dbs;i++){
...
static int leaf_find(const struct DB *db, uint32_t dbi, const void *pg,
                     const void *key, uint32_t klen, int *found)
{
...
static int int_find_child(const struct DB *db, uint32_t dbi, const void *pg,
...
static uint32_t leaf_split(struct Txn *txn, uint32_t dbi,
                            ...
                            uint16_t *sep_klen_out)
{
    ...
    for (int j=0;j<total;j++){
        ...
        leaf_insert(dst,dpos,kbuf+koff[j],(uint16_t)kl2[j],
                             vbuf+voff[j],(uint16_t)vl2[j],NULL);
    }
...
int txn_put_flags_dbi(Txn *txn_pub, uint32_t dbi,
                      const void *key, uint32_t key_len,
                      ...
                      unsigned flags, void **reserved_out)
{
    ...
    struct DB *db=txn->db;
    if (dbi>=db->num_dbs) return SAP_ERROR;
    ...
    if (is_dupsort) {
        if (flags&SAP_RESERVE) return SAP_ERROR; /* incompatible */
        uint32_t comp_len=4+key_len+val_len;
        if (comp_len>UINT16_MAX) return SAP_FULL;
        comp_buf=(uint8_t*)txn_scratch_alloc(txn,comp_len);
        if (!comp_buf) return SAP_ERROR;
        wr32(comp_buf,key_len);
        memcpy(comp_buf+4,key,key_len);
        memcpy(comp_buf+4+key_len,val,val_len);
        key=comp_buf; key_len=comp_len;
        val=""; val_len=0;
        flags|=SAP_NOOVERWRITE; /* exact dup rejection */
    }

    if (key_len>UINT16_MAX||val_len>UINT16_MAX){ txn_scratch_release(txn,scratch_mark); return SAP_FULL; }
    void **rout=(flags&SAP_RESERVE)?reserved_out:NULL;

    if (SLOT_SZ+LCELL_SZ(key_len,val_len)+LEAF_HDR>db->page_size){ txn_scratch_release(txn,scratch_mark); return SAP_FULL; }

    int rc;

    if (txn->dbs[dbi].root_pgno==INVALID_PGNO) {
        uint32_t pgno=txn_alloc(txn);
        if (pgno==INVALID_PGNO){ rc=SAP_ERROR; goto cleanup; }
        pg_init_leaf(db->pages[pgno],pgno,db->page_size);
        if (leaf_insert(db->pages[pgno],0,key,(uint16_t)key_len,val,(uint16_t)val_len,rout)<0)
            { rc=SAP_ERROR; goto cleanup; }
        txn->dbs[dbi].root_pgno=pgno;
        txn->dbs[dbi].num_entries++;
        changed=1;
        rc=SAP_OK; goto cleanup;
    }
    ...
    while (PG_TYPE(db->pages[pgno])==PAGE_INTERNAL) {
        ...
        if (depth>=MAX_DEPTH-1){ rc=SAP_ERROR; goto cleanup; }
    ...
    /* NOOVERWRITE: check on pre-COW leaf */
    {
        ...
        int pre_found; leaf_find(db,dbi,pre_lpg,key,key_len,&pre_found);
        if (pre_found&&(flags&SAP_NOOVERWRITE)){ rc=SAP_EXISTS; goto cleanup; }
    }

    /* COW leaf */
    uint32_t leaf_pgno=txn_cow(txn,pgno);
    if (leaf_pgno==INVALID_PGNO){ rc=SAP_ERROR; goto cleanup; }
    ...
    if (depth>0) {
        ...
        for (int d=depth-1; d>=0; d--) {
            ...
            if (pp==INVALID_PGNO){ rc=SAP_ERROR; goto cleanup; }
            path[d]=pp;
    ...
    int found; int pos=leaf_find(db,dbi,lpg,key,key_len,&found);
    int is_update=found;
    if (found) { leaf_remove(lpg,pos); pos=leaf_find(db,dbi,lpg,key,key_len,&found); }

    if (leaf_insert(lpg,pos,key,(uint16_t)key_len,val,(uint16_t)val_len,rout)==0) {
        if (!is_update) txn->dbs[dbi].num_entries++;
        changed=1;
        rc=SAP_OK; goto cleanup;
    }
    ...
    if (!sep_buf){ rc=SAP_ERROR; goto cleanup; }
    uint16_t sep_klen;
    uint32_t rpgno=leaf_split(txn,dbi,leaf_pgno,lpg,
                               key,(uint16_t)key_len,val,(uint16_t)val_len,
                               sep_buf,&sep_klen);
    if (rpgno==INVALID_PGNO){ rc=SAP_ERROR; goto cleanup; }
    if (!is_update) txn->dbs[dbi].num_entries++;

    /* For RESERVE: locate the entry in the result pages */
    if (rout) {
        int f; int p=leaf_find(db,dbi,db->pages[leaf_pgno],key,key_len,&f);
        if (!f) { p=leaf_find(db,dbi,db->pages[rpgno],key,key_len,&f); lpg=db->pages[rpgno]; }
        else { lpg=db->pages[leaf_pgno]; }
        if (f) {
    ...
    for (int d=depth-1; d>=0; d--) {
        ...
        if (!psep_buf) {
            ...
            if (!psep_buf){ rc=SAP_ERROR; goto cleanup; }
        }
        uint16_t psep_klen;
        uint32_t nr=int_split(txn,par_pgno,par,ins_pos,sep_key,sep_klen,right_pgno,
                               psep_buf,&psep_klen);
        if (nr==INVALID_PGNO){ rc=SAP_ERROR; goto cleanup; }
        memcpy(sep_buf,psep_buf,psep_klen); sep_klen=psep_klen; sep_key=sep_buf;
        left_pgno=par_pgno; right_pgno=nr;
    }

    /* Root split: new root */
    uint32_t new_root=txn_alloc(txn);
    if (new_root==INVALID_PGNO){ rc=SAP_ERROR; goto cleanup; }
...
cleanup:
    ...
    if (is_dupsort && rc==SAP_EXISTS) rc=SAP_OK;
    return rc;
}

int txn_put_flags(Txn *txn, const void *key, uint32_t key_len,
                  const void *val, uint32_t val_len,
                  unsigned flags, void **reserved_out)
{ return txn_put_flags_dbi(txn,0,key,key_len,val,val_len,flags,reserved_out); }

int txn_put(Txn *txn, const void *key, uint32_t key_len,
            const void *val, uint32_t val_len)
{ return txn_put_flags_dbi(txn,0,key,key_len,val,val_len,0,NULL); }

int txn_put_dbi(Txn *txn, uint32_t dbi, const void *key, uint32_t key_len,
                const void *val, uint32_t val_len)
{ return txn_put_flags_dbi(txn,dbi,key,key_len,val,val_len,0,NULL); }

int txn_put_if(Txn *txn_pub, uint32_t dbi,
               const void *key, uint32_t key_len,
               const void *val, uint32_t val_len,
               const void *expected_val, uint32_t expected_len)
{
    ...
    if (txn->flags&TXN_RDONLY) return SAP_READONLY;
    if (dbi>=txn->db->num_dbs) return SAP_ERROR;
    if (txn->db->dbs[dbi].flags&DBI_DUPSORT) return SAP_ERROR;
    if (!expected_val&&expected_len>0) return SAP_ERROR;

    rc=txn_get_dbi((Txn*)txn,dbi,key,key_len,&cur_val,&cur_len);
    if (rc!=SAP_OK) return rc;
    if (cur_len!=expected_len) return SAP_CONFLICT;
    if (expected_len&&memcmp(cur_val,expected_val,expected_len)!=0) return SAP_CONFLICT;
...
int txn_get_dbi(Txn *txn_pub, uint32_t dbi,
                const void *key, uint32_t key_len,
                const void **val_out, uint32_t *val_len_out)
{
    struct Txn *txn=(struct Txn*)txn_pub;
    struct DB *db=txn->db;
    if (dbi>=db->num_dbs) return SAP_NOTFOUND;

    /* DUPSORT: encode prefix composite [key_len:4][key] to find first dup */
    if (db->dbs[dbi].flags&DBI_DUPSORT) {
        struct ScratchMark scratch_mark=txn_scratch_mark(txn);
        uint32_t comp_len=4+key_len;
        if (comp_len>UINT16_MAX) return SAP_NOTFOUND;
        uint8_t *comp=(uint8_t*)txn_scratch_alloc(txn,comp_len);
        if (!comp) return SAP_ERROR;
        wr32(comp,key_len);
        memcpy(comp+4,key,key_len);
        /* Search: finds first composite >= [klen][key] */
        if (txn->dbs[dbi].root_pgno==INVALID_PGNO){
            txn_scratch_release(txn,scratch_mark);
            return SAP_NOTFOUND;
        }
        ...
        while (PG_TYPE(db->pages[pgno])==PAGE_INTERNAL){
            void *pg=db->pages[pgno];
            pgno=int_child(pg,int_find_child(db,dbi,pg,comp,comp_len));
        }
        void *lpg=db->pages[pgno];
        int found; int pos=leaf_find(db,dbi,lpg,comp,comp_len,&found);
        txn_scratch_release(txn,scratch_mark);
        /* found=1 only if exact composite match (empty val). For first dup,
         * check if entry at pos has matching key portion. */
        if (pos>=(int)PG_NUM(lpg)) return SAP_NOTFOUND;
        uint16_t off=(uint16_t)L_SLOT(lpg,pos);
        uint16_t stored_klen=L_CKLEN(lpg,off);
        const uint8_t *stored_key=L_CKEY(lpg,off);
        /* Decode composite: first 4 bytes = original key_len */
        if (stored_klen<4) return SAP_NOTFOUND;
        uint32_t orig_kl=rd32(stored_key);
        if (orig_kl!=key_len) return SAP_NOTFOUND;
        /* Compare key portion */
        int c;
        if (db->dbs[dbi].cmp)
            c=db->dbs[dbi].cmp(stored_key+4,orig_kl,key,key_len,db->dbs[dbi].cmp_ctx);
        else
            c=default_cmp(stored_key+4,orig_kl,key,key_len);
        if (c!=0) return SAP_NOTFOUND;
        /* Decode value from composite */
        *val_out=stored_key+4+orig_kl;
        *val_len_out=stored_klen-4-orig_kl;
        return SAP_OK;
    }

    if (key_len>UINT16_MAX) return SAP_NOTFOUND;
    if (txn->dbs[dbi].root_pgno==INVALID_PGNO) return SAP_NOTFOUND;
    uint32_t pgno=txn->dbs[dbi].root_pgno;
    while (PG_TYPE(db->pages[pgno])==PAGE_INTERNAL){
        void *pg=db->pages[pgno];
...
int txn_get(Txn *txn, const void *key, uint32_t key_len,
            const void **val_out, uint32_t *val_len_out)
...
int txn_del_dbi(Txn *txn_pub, uint32_t dbi, const void *key, uint32_t key_len)
{
    ...
    if (depth>0) txn->dbs[dbi].root_pgno=path[0]; else txn->dbs[dbi].root_pgno=leaf_pgno;

    if (PG_NUM(lpg)>0) return SAP_OK;

    txn_free_page(txn,leaf_pgno);

    if (depth==0){ txn->dbs[dbi].root_pgno=INVALID_PGNO; return SAP_OK; }
    ...
    for (int d=depth-1; d>=0; d--){
        ...
        if (d==0){ txn->dbs[dbi].root_pgno=sole; break; }
...
static int txn_mark_tree_old(struct Txn *txn, uint32_t root_pgno)
{
    ...
    if (!txn||root_pgno==INVALID_PGNO) return 0;
    ...
    while (top) {
        ...
        if (PG_TYPE(pg)==PAGE_INTERNAL) {
            ...
            uint32_t need=n+1;
            if (top>UINT32_MAX-need) { rc=-1; break; }
            if (top+need>cap) {
                uint32_t nc=cap;
                while (nc<top+need) {
                    if (nc>UINT32_MAX/2) { rc=-1; break; }
                    nc*=2;
                }
                if (rc) break;
...
static int txn_load_sorted_empty_fast(struct Txn *txn, uint32_t dbi,
                                      ...
                                      uint32_t count, int is_dupsort)
{
    ...
    for (uint32_t i=0;i<count;i++) {
        for (;;) {
            ...
            if (leaf_insert(leaf,(int)PG_NUM(leaf),
                            store_key,store_klen,store_val,store_vlen,NULL)==0) {
                if (PG_NUM(leaf)==1) {
    ...
    while (cur_count>1) {
        ...
        if (!cap||!choice||!feasible) {
            free(cap); free(choice); free(feasible);
            rc=SAP_ERROR;
            goto cleanup;
        }
        ...
        for (uint32_t i=cur_count;i-->0;) {
            ...
            if (max_group>remaining) max_group=remaining;
        ...
        while (idx<cur_count) {
            ...
            for (uint32_t j=1;j<group;j++) {
                if (int_insert(ipg,(int)PG_NUM(ipg),
                               cur[idx+j].min_key,cur[idx+j].min_len,
                               cur[idx+j].pgno)<0) {
                    free(cap); free(choice); free(feasible);
                    rc=SAP_ERROR;
                    goto cleanup;
                                               ...
                                               uint32_t count)
{
    ...
    if (existing64>UINT32_MAX) return SAP_ERROR;
    max_total64=existing64+(uint64_t)count;
    if (max_total64>UINT32_MAX) return SAP_ERROR;
    max_total=(uint32_t)max_total64;

    if (max_total==0) return SAP_OK;
    if ((size_t)max_total > SIZE_MAX/sizeof(*mkeys)) return SAP_ERROR;
    ...
    if (!mkeys||!mvals||!mklen||!mvlen) goto cleanup;

    cur=cursor_open_dbi((Txn*)txn,dbi);
    if (!cur) goto cleanup;
    rc=cursor_first(cur);
    if (rc==SAP_OK) has_old=1;
    else if (rc==SAP_NOTFOUND) has_old=0;
    else goto cleanup;

    while (has_old&&in_i<count) {
        const void *ok,*ov; uint32_t okl,ovl;
        const void *nk=keys[in_i] ? keys[in_i] : &zero;
        const void *nv=vals[in_i] ? vals[in_i] : &zero;
        int c;

        rc=cursor_get(cur,&ok,&okl,&ov,&ovl);
        if (rc!=SAP_OK) goto cleanup;

        c=user_keycmp(db,dbi,ok,okl,nk,key_lens[in_i]);
        if (c<0) {
            mkeys[out_i]=ok; mklen[out_i]=okl;
            mvals[out_i]=ov; mvlen[out_i]=ovl;
            out_i++;
            rc=cursor_next(cur);
            if (rc==SAP_OK) has_old=1;
            else if (rc==SAP_NOTFOUND) has_old=0;
            ...
            mkeys[out_i]=nk; mklen[out_i]=key_lens[in_i];
            mvals[out_i]=nv; mvlen[out_i]=val_lens[in_i];
            out_i++;
            in_i++;
        } else {
            mkeys[out_i]=nk; mklen[out_i]=key_lens[in_i];
...
cleanup:
    if (cur) cursor_close(cur);
    free(mkeys); free(mvals); free(mklen); free(mvlen);
    return rc;
}

int txn_load_sorted(Txn *txn_pub, uint32_t dbi,
                    const void *const *keys, const uint32_t *key_lens,
                    const void *const *vals, const uint32_t *val_lens,
                    uint32_t count)
{
    ...
    if (dbi>=db->num_dbs) return SAP_ERROR;
    if (count==0) return SAP_OK;
    if (!key_lens||!val_lens) return SAP_ERROR;
    if (!keys||!vals) return SAP_ERROR;
    ...
    for (uint32_t i=0;i<count;i++) {
        ...
        if (key_lens[i]>UINT16_MAX||val_lens[i]>UINT16_MAX) return SAP_FULL;
        if (!kcur&&key_lens[i]>0) return SAP_ERROR;
        if (!vcur&&val_lens[i]>0) return SAP_ERROR;
        if (!kcur) kcur=&zero;
        if (!vcur) vcur=&zero;
        if (i==0) continue;
        ...
        if (kc>0) return SAP_ERROR;
        if (kc==0 && !is_dupsort) return SAP_EXISTS;
    ...
    if (txn->dbs[dbi].root_pgno==INVALID_PGNO) {
        ...
        int rc;
        if (!child) return SAP_ERROR;
        rc=txn_load_sorted_empty_fast((struct Txn*)child,dbi,
                                      keys,key_lens,vals,val_lens,count,is_dupsort);
        if (rc==SAP_OK) {
            for (uint32_t i=0;i<count;i++) {
                const void *k=keys[i] ? keys[i] : &zero;
    ...
    if (!is_dupsort&&txn->new_cnt==0&&txn->old_cnt==0) {
        ...
        int rc;
        if (!child) return SAP_ERROR;
        rc=txn_load_sorted_nonempty_merge_fast((struct Txn*)child,dbi,
                                               keys,key_lens,vals,val_lens,count);
        if (rc==SAP_OK) {
            for (uint32_t i=0;i<count;i++) {
                const void *k=keys[i] ? keys[i] : &zero;
                (void)txn_track_change((struct Txn*)child,dbi,k,key_lens[i]);
...
int txn_count_range(Txn *txn_pub, uint32_t dbi,
                    ...
                    uint64_t *count_out)
{
    ...
    if (!txn||!count_out) return SAP_ERROR;
    db=txn->db;
    if (dbi>=db->num_dbs) return SAP_ERROR;
    if (!lo&&lo_len>0) return SAP_ERROR;
    if (!hi&&hi_len>0) return SAP_ERROR;
    *count_out=0;
    ...
    if (has_lo&&has_hi) {
        ...
        if (cmp>=0) return SAP_OK;
    }

    cur=cursor_open_dbi((Txn*)txn,dbi);
    if (!cur) return SAP_ERROR;

    if (!has_lo) {
        rc=cursor_first(cur);
    } else if (is_dupsort) {
        struct ScratchMark scratch_mark=txn_scratch_mark(txn);
        uint32_t comp_len=4u+lo_len;
        uint8_t *comp;
        if (comp_len>UINT16_MAX) {
            cursor_close(cur);
            return SAP_OK;
        ...
        wr32(comp,lo_len);
        memcpy(comp+4,lo,lo_len);
        rc=cursor_seek(cur,comp,comp_len);
        txn_scratch_release(txn,scratch_mark);
    } else {
    ...
    for (;;) {
        ...
        if (rc==SAP_NOTFOUND) { rc=SAP_OK; break; }
        if (rc!=SAP_OK) break;
...
Txn *txn_begin(DB *db_pub, Txn *par_pub, unsigned int flags)
{
    ...
    if (!txn) {
        if (!par) SAP_MUTEX_UNLOCK(db->write_mutex);
        return NULL;
    }
    ...
    if (par){
        ...
        txn->free_pgno=par->free_pgno; txn->num_pages=par->num_pages;
        txn->saved_free=par->free_pgno; txn->saved_npages=par->num_pages;
    } else {
        txn->txnid=db->txnid;
        for (uint32_t i=0;i<nd;i++){
        ...
        txn->free_pgno=db->free_pgno; txn->num_pages=db->num_pages;
        txn->saved_free=db->free_pgno; txn->saved_npages=db->num_pages;
        if (!(flags&TXN_RDONLY)){
            db->write_txn=txn;
            ...
            if (db->num_readers >= db->cap_readers) {
                ...
                uint64_t *na = (uint64_t *)realloc(db->active_readers,
                                                    nc * sizeof(uint64_t));
                if (na) { db->active_readers = na; db->cap_readers = nc; }
            }
            if (db->num_readers < db->cap_readers)
...
int txn_commit(Txn *txn_pub)
{
    ...
    if (txn->parent){
        ...
        par->free_pgno=txn->free_pgno; par->num_pages=txn->num_pages;
...
DB *db_open(PageAllocator *alloc, uint32_t page_size,
            keycmp_fn cmp, void *cmp_ctx)
{
    uint32_t max_dbs;
    if (!alloc||!alloc->alloc_page||!alloc->free_page) return NULL;
    if (page_size<256 || page_size>UINT16_MAX) return NULL;
    max_dbs=meta_max_dbs(page_size);
    if (max_dbs==0) return NULL;
    ...
    db->dbs[0].cmp=cmp; db->dbs[0].cmp_ctx=cmp_ctx;
...
int dbi_open(DB *db_pub, uint32_t dbi, keycmp_fn cmp, void *cmp_ctx,
             unsigned flags)
{
    struct DB *db=(struct DB*)db_pub;
    uint32_t max_dbs=meta_max_dbs(db->page_size);
    if (max_dbs>SAP_MAX_DBI) max_dbs=SAP_MAX_DBI;
    if (dbi>=max_dbs) return SAP_ERROR;
    ...
    if (dbi>=db->num_dbs) {
        for (uint32_t i=db->num_dbs;i<=dbi;i++){
            ...
            db->dbs[i].cmp=NULL; db->dbs[i].cmp_ctx=NULL;
            db->dbs[i].vcmp=NULL; db->dbs[i].vcmp_ctx=NULL;
            db->dbs[i].flags=0;
        }
        db->num_dbs=dbi+1;
    }
    db->dbs[dbi].cmp=cmp; db->dbs[dbi].cmp_ctx=cmp_ctx;
...
int dbi_set_dupsort(DB *db_pub, uint32_t dbi, keycmp_fn vcmp, void *vcmp_ctx)
{
    ...
    db->dbs[dbi].vcmp=vcmp; db->dbs[dbi].vcmp_ctx=vcmp_ctx;
    SAP_MUTEX_UNLOCK(db->write_mutex);
    return SAP_OK;
}
...
int db_restore(DB *db_pub, sap_read_fn reader, void *ctx)
{
    ...
    if (snap_magic!=SNAP_MAGIC||snap_version!=SNAP_VERSION||
        snap_psz!=db->page_size||snap_npages<2){
        SAP_MUTEX_UNLOCK(db->reader_mutex);
        SAP_MUTEX_UNLOCK(db->write_mutex);
        return SAP_ERROR;
    }
    ...
    for (loaded=0; loaded<snap_npages; loaded++){
        ...
        if (!pg||reader(pg,db->page_size,ctx)!=0){
            if (pg) db->alloc->free_page(db->alloc->ctx,pg,db->page_size);
...

    {
        ...
        if (old_pages){
            ...
            for (uint32_t i=0;i<lim;i++)
                if (old_pages[i]) db->alloc->free_page(db->alloc->ctx,old_pages[i],db->page_size);
            free(old_pages);
        }
...
static int watch_same(const struct WatchRec *wr, uint32_t dbi,
                      const void *prefix, uint32_t prefix_len,
...
int db_watch_dbi(DB *db_pub, uint32_t dbi,
                 const void *prefix, uint32_t prefix_len,
                 sap_watch_fn cb, void *ctx)
{
    struct DB *db=(struct DB*)db_pub;
    struct WatchRec *wr;
    if (!db||!cb) return SAP_ERROR;
    if (!prefix&&prefix_len>0) return SAP_ERROR;
    ...
    if (db->num_watches>=db->cap_watches) {
        ...
        struct WatchRec *nw=(struct WatchRec*)realloc(db->watches,
                                                      (size_t)nc*sizeof(*nw));
        if (!nw) {
            SAP_MUTEX_UNLOCK(db->write_mutex);
            return SAP_ERROR;
...
int db_unwatch_dbi(DB *db_pub, uint32_t dbi,
                   const void *prefix, uint32_t prefix_len,
                   sap_watch_fn cb, void *ctx)
{
    struct DB *db=(struct DB*)db_pub;
...
int db_watch(DB *db_pub, const void *prefix, uint32_t prefix_len,
             sap_watch_fn cb, void *ctx)
{
    return db_watch_dbi(db_pub,0,prefix,prefix_len,cb,ctx);
}

int db_unwatch(DB *db_pub, const void *prefix, uint32_t prefix_len,
               sap_watch_fn cb, void *ctx)
{
...
{
    if (root_pgno==INVALID_PGNO) return 0;
    uint32_t d=1, pgno=root_pgno;
    while (PG_TYPE(db->pages[pgno])==PAGE_INTERNAL){
        pgno=I_LEFT(db->pages[pgno]); d++;
    }
    return d;
}
...
{
    if (!db_pub||!stat) return SAP_ERROR;
    struct DB *db=(struct DB*)db_pub;
    SAP_MUTEX_LOCK(db->write_mutex);
...
{
    if (!txn_pub||!stat) return SAP_ERROR;
    struct Txn *txn=(struct Txn*)txn_pub;
    struct DB *db=txn->db;
...
{
    struct Txn *txn=(struct Txn*)txn_pub;
    if (!txn||dbi>=txn->db->num_dbs) return NULL;
    struct Cursor *c=(struct Cursor*)calloc(1,sizeof(*c));
    if (!c) return NULL;
    c->txn=txn; c->dbi=dbi; c->depth=-1;
    return (Cursor*)c;
}

Cursor *cursor_open(Txn *txn_pub)
{
    return cursor_open_dbi(txn_pub,0);
}
...
int cursor_renew(Cursor *cp, Txn *txn_pub)
{
    ...
    struct Txn *txn=(struct Txn*)txn_pub;
    if (!c||!txn) return SAP_ERROR;
    if (c->txn&&c->txn->db!=txn->db) return SAP_ERROR;
    if (c->dbi>=txn->db->num_dbs) return SAP_ERROR;
    c->txn=txn;
    c->depth=-1;
...
int cursor_first(Cursor *cp)
{
    ...
    uint32_t dbi=c->dbi;
    if (txn->dbs[dbi].root_pgno==INVALID_PGNO){ c->depth=-1; return SAP_NOTFOUND; }
    c->depth=0;
               ...
               const void **val_out,  uint32_t *val_len_out)
{
    ...
    if (db->dbs[c->dbi].flags&DBI_DUPSORT) {
        ...
        if (klen<4) return SAP_ERROR;
        uint32_t orig_kl=rd32(comp);
        *key_out=comp+4; *key_len_out=orig_kl;
        *val_out=comp+4+orig_kl; *val_len_out=klen-4-orig_kl;
    } else {
...
{
    struct Cursor *c=(struct Cursor*)cp;
    if (!c||!key_out||!key_len_out) return SAP_ERROR;
    if (c->depth<0) return SAP_NOTFOUND;
    struct DB *db=c->txn->db;
    void *lpg=db->pages[c->stack[c->depth]];
    int pos=c->idx[c->depth];
    ...
    if (db->dbs[c->dbi].flags&DBI_DUPSORT) {
        ...
        if (klen<4) return SAP_ERROR;
        uint32_t orig_kl=rd32(comp);
        if (orig_kl>(uint32_t)(klen-4)) return SAP_ERROR;
        *key_out=comp+4;
        *key_len_out=orig_kl;
    } else {
...
static uint32_t cow_path(struct Cursor *c)
{
    ...
    if (leaf_pgno==INVALID_PGNO) return INVALID_PGNO;
    c->stack[depth]=leaf_pgno;

    uint32_t child=leaf_pgno;
    for (int d=depth-1; d>=0; d--) {
        uint32_t pp=txn_cow(txn,c->stack[d]);
        if (pp==INVALID_PGNO) return INVALID_PGNO;
        c->stack[d]=pp;
        void *par=db->pages[pp];
        int ci=c->idx[d];
        if (ci==0) SET_I_LEFT(par,child);
    ...
    if (depth>0) txn->dbs[dbi].root_pgno=c->stack[0];
    else         txn->dbs[dbi].root_pgno=leaf_pgno;
    return leaf_pgno;
}
...
int cursor_put(Cursor *cp, const void *val, uint32_t val_len, unsigned flags)
{
    ...
    struct ScratchMark scratch_mark=txn_scratch_mark(txn);
    if (txn->flags&TXN_RDONLY) return SAP_READONLY;
    if (val_len>UINT16_MAX) return SAP_FULL;
    struct DB *db=txn->db;
    uint32_t dbi=c->dbi;

    void *orig_lpg=db->pages[c->stack[c->depth]];
    int pos=c->idx[c->depth];
    if (pos<0||pos>=(int)PG_NUM(orig_lpg)) return SAP_NOTFOUND;
...
int cursor_del(Cursor *cp)
{
    ...
    if (PG_NUM(lpg)==0) {
        ...
        for (int d=depth-1; d>=0; d--){
            ...
            txn_free_page(txn,par_pgno);
            if (d==0){ txn->dbs[dbi].root_pgno=sole; break; }
            uint32_t gp=c->stack[d-1]; void *gpg=db->pages[gp];
            int gc=c->idx[d-1];
            if (gc==0) SET_I_LEFT(gpg,sole); else SET_I_CRIGHT(gpg,I_SLOT(gpg,gc-1),sole);
            break;
        }
...
static const uint8_t *dup_cur_key(struct Cursor *c, uint32_t *kl_out)
{
    ...
    void *lpg=db->pages[c->stack[c->depth]];
    int pos=c->idx[c->depth];
    if (pos<0||pos>=(int)PG_NUM(lpg)) return NULL;
    uint16_t off=(uint16_t)L_SLOT(lpg,pos);
    uint16_t comp_klen=L_CKLEN(lpg,off);
    if (comp_klen<4) return NULL;
    const uint8_t *comp=L_CKEY(lpg,off);
    *kl_out=rd32(comp);
...
static int dup_same_key(struct Cursor *c, const void *saved_key, uint32_t saved_kl)
{
    ...
    if (!cur_key||cur_kl!=saved_kl) return 0;
    struct DB *db=c->txn->db;
    if (db->dbs[c->dbi].cmp)
        return db->dbs[c->dbi].cmp(cur_key,cur_kl,saved_key,saved_kl,db->dbs[c->dbi].cmp_ctx)==0;
...
int cursor_count_dup(Cursor *cp, uint64_t *count)
{
    ...
    struct ScratchMark scratch_mark=txn_scratch_mark(c->txn);
    if (!(db->dbs[c->dbi].flags&DBI_DUPSORT)) return SAP_ERROR;
    if (!count) return SAP_ERROR;
    /* Save position, go to first_dup, count forward */
    uint32_t saved_kl;
    const uint8_t *saved_key=dup_cur_key(c,&saved_kl);
    if (!saved_key){ *count=0; return SAP_OK; }
    uint8_t *kbuf=txn_scratch_copy(c->txn,saved_key,saved_kl);
    if (!kbuf) return SAP_ERROR;
    ...
    uint64_t cnt=1;
    while (cursor_next(cp)==SAP_OK) {
        if (!dup_same_key(c,kbuf,saved_kl)) break;
        cnt++;
    }
    *count=cnt;
...
int cursor_seek_prefix(Cursor *cp, const void *prefix, uint32_t prefix_len)
{
    ...
    if (db->dbs[c->dbi].flags&DBI_DUPSORT) {
        struct ScratchMark scratch_mark=txn_scratch_mark(c->txn);
        /* Encode prefix as composite: [prefix_len:4][prefix] */
        uint32_t comp_len=4+prefix_len;
        uint8_t *comp=(uint8_t*)txn_scratch_alloc(c->txn,comp_len);
        if (!comp) return SAP_ERROR;
        wr32(comp,prefix_len);
        memcpy(comp+4,prefix,prefix_len);
        int rc=cursor_seek(cp,comp,comp_len);
        txn_scratch_release(c->txn,scratch_mark);
        if (rc!=SAP_OK) return rc;
        /* Verify the found key starts with prefix */
    ...
    if (cursor_get(cp,&k,&kl,&v,&vl)!=SAP_OK) return SAP_NOTFOUND;
...
{
    const void *k,*v; uint32_t kl,vl;
    if (cursor_get(cp,&k,&kl,&v,&vl)!=SAP_OK) return 0;
    if (kl<prefix_len) return 0;
