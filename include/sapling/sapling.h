...
/* ------------------------------------------------------------------ */
typedef int (*keycmp_fn)(const void *a, uint32_t a_len,
...
int  dbi_open      (DB *db, uint32_t dbi, keycmp_fn cmp, void *cmp_ctx,
                    unsigned flags);
...
/* ------------------------------------------------------------------ */
int txn_get_dbi(Txn *txn, uint32_t dbi,
                const void *key, uint32_t key_len,
                const void **val_out, uint32_t *val_len_out);

int txn_put_dbi(Txn *txn, uint32_t dbi,
                const void *key, uint32_t key_len,
                const void *val, uint32_t val_len);

int txn_put_flags_dbi(Txn *txn, uint32_t dbi,
                      const void *key, uint32_t key_len,
                      const void *val, uint32_t val_len,
                      unsigned flags, void **reserved_out);
...
int txn_del_dbi(Txn *txn, uint32_t dbi,
                const void *key, uint32_t key_len);
 ...
 */
int txn_load_sorted(Txn *txn, uint32_t dbi,
 ...
 */
int txn_count_range(Txn *txn, uint32_t dbi,
...
int cursor_get_key(Cursor *cursor,
                   const void **key_out, uint32_t *key_len_out);

/* ------------------------------------------------------------------ */
/* Cursor mutations                                                     */
/* ------------------------------------------------------------------ */
int cursor_put(Cursor *cursor, const void *val, uint32_t val_len,
               unsigned flags);
...
/* ------------------------------------------------------------------ */
int cursor_seek_prefix(Cursor *cursor,
                       const void *prefix, uint32_t prefix_len);
int cursor_in_prefix  (Cursor *cursor,
                       const void *prefix, uint32_t prefix_len);
