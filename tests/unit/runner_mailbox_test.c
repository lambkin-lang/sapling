...
static int test_claim_busy_and_takeover(void)
{
    ...
    CHECK(db != NULL);
    CHECK(inbox_put(db, 7u, 1u, frame, sizeof(frame)) == ERR_OK);
...
static int test_ack_removes_inbox_and_lease(void)
{
    ...
    CHECK(db != NULL);
    CHECK(inbox_put(db, 9u, 5u, frame, sizeof(frame)) == ERR_OK);
...
static int test_ack_rejects_stale_lease_token(void)
{
    ...
    CHECK(db != NULL);
    CHECK(inbox_put(db, 3u, 11u, frame, sizeof(frame)) == ERR_OK);
...
static int test_requeue_moves_message_and_clears_lease(void)
{
    ...
    CHECK(db != NULL);
    CHECK(inbox_put(db, 12u, 50u, frame, sizeof(frame)) == ERR_OK);
    ...
    CHECK(inbox_get(db, 12u, 60u, &new_val, &new_len) == ERR_OK);
